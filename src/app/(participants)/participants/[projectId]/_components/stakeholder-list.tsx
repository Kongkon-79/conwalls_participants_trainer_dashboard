...
export default function StakeholderList({
  ...
  const [triggerStakeholder, setTriggerStakeholder] =
    useState<Stakeholder | null>(null);
  const [measuresStakeholder, setMeasuresStakeholder] =
    useState<Stakeholder | null>(null);
  const [deleteStakeholderId, setDeleteStakeholderId] = useState<string | null>(
    null,
  );
  ...
  if (triggerStakeholder) {
    return (
      <TriggerForm
        stakeholder={triggerStakeholder}
        onBack={() => {
    ...
    <div className="space-y-6 pb-10">
            ...
            >
              <div className="flex items-center gap-4 flex-1">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-[8px] bg-[#BADA55] flex items-center justify-center p-1">
                    <User size={20} className="text-[#00253E]" />
                  </div>
                  {editingNameId === sh._id ? (
                    ...
                    <span className="text-lg md:text-xl  font-semibold text-[#00253E] leading-[110%]">
                      {sh.name}
                    </span>
                  )}
                </div>
                ...
                {editingNameId !== sh._id && (
                  <div className="flex items-center gap-2 ml-4">
                    <button
                      onClick={() => {
                        setEditingNameId(sh._id);
                        setEditNameValue(sh.name);
                      }}
                      className="p-2 bg-gray-100 rounded-[8px]"
                    >
                      <Pencil className="w-4 h-4 text-[#00253E]" />
                    </button>
                    <button
                      onClick={() => setDeleteStakeholderId(sh._id)}
                      className="p-2 bg-gray-100 rounded-[8px]"
                    >
                      <Trash2 className="w-4 h-4 text-[#00253E]" />
                    </button>
                  </div>
                )}
              </div>
              ...
              {editingNameId !== sh._id && (
                <div className="flex items-center gap-6">
                  <button
                    ...
                    className="flex items-center gap-2 notranslate text-[16px] md:text-lg font-semibold text-[#00253E] hover:text-primary transition-colors"
                  >
                    Trigger
                    <span className="w-7 h-7 rounded-[8px] bg-[#BADA55] flex items-center justify-center p-1">
                      <AlarmClockPlus size={20} className="text-[#00253E]" />
                    </span>
                  </button>
                  ...
                  >
                    {lang === "de" ? "Maßnahmen" : "Measures"}
                     <span className="w-7 h-7 rounded-[8px] bg-[#BADA55] flex items-center justify-center p-1">
                      <CopyMinus size={20} className="text-[#00253E]" />
                    </span>
                  </button>
                </div>
              )}
      ...
      <div className="flex items-center justify-between pt-6">
        ...
        >
          ...
          {lang === "de"
            ? "weitere Zielgruppen hinzufügen"
      ...
      <Dialog
        open={!!deleteStakeholderId}
        onOpenChange={(open) => !open && setDeleteStakeholderId(null)}
      >
        <DialogContent className="sm:max-w-[425px] bg-white">
          <DialogHeader>
            <DialogTitle className="text-[#00253E]">
              Delete Stakeholder
            </DialogTitle>
          ...
          <DialogFooter>
            <Button
              ...
              onClick={() => setDeleteStakeholderId(null)}
            >
              Cancel
            </Button>
            <Button
